/**
 * @file VkBloomCombinePipeline.cpp
 * @brief Combine pass: scene HDR + bloom*intensity (M08.2).
 */
...
VkBloomCombinePipeline::~VkBloomCombinePipeline() { Shutdown(); }

bool VkBloomCombinePipeline::Init(VkDevice device, VkRenderPass renderPass,
    ...
    if (vertModule == VK_NULL_HANDLE || fragModule == VK_NULL_HANDLE) {
        ...
        LOG_ERROR(Render, "VkBloomCombinePipeline: failed to create shader modules");
        return false;
    }
    ...
    if (vkCreateDescriptorSetLayout(device, &dslci, nullptr, &m_setLayout) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkBloomCombinePipeline: descriptor set layout failed");
        return false;
    }
    ...
    pushConstant.size = 4; // intensity (float)
    ...
    if (vkCreatePipelineLayout(device, &plci, nullptr, &m_layout) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkBloomCombinePipeline: pipeline layout failed");
        return false;
    }
    ...
    if (vkCreateDescriptorPool(device, &dpci, nullptr, &m_descriptorPool) != VK_SUCCESS) {
        ...
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomCombinePipeline: descriptor pool failed");
        return false;
    }
    ...
    if (vkAllocateDescriptorSets(device, &dsai, &m_descriptorSet) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomCombinePipeline: allocate descriptor set failed");
        return false;
    }
    ...
    if (vkCreateSampler(device, &sci, nullptr, &m_sampler) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomCombinePipeline: sampler failed");
        return false;
    }
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &gpci, nullptr, &m_pipeline) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomCombinePipeline: graphics pipeline failed");
        return false;
    }
...
void VkBloomCombinePipeline::SetInputs(VkImageView sceneHdrView, VkImageView bloomView) {
    ...
    imageInfo[0].sampler = m_sampler;
    imageInfo[0].imageView = sceneHdrView;
    imageInfo[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    imageInfo[1].sampler = m_sampler;
    imageInfo[1].imageView = bloomView;
    ...
    writes[1].pImageInfo = &imageInfo[1];
    vkUpdateDescriptorSets(m_device, 2, writes.data(), 0, nullptr);
}

void VkBloomCombinePipeline::Shutdown() {
