/**
 * @file VkBloomPrefilterPipeline.cpp
 * @brief Bloom prefilter: sample HDR, soft threshold + knee.
 */
...
VkBloomPrefilterPipeline::~VkBloomPrefilterPipeline() { Shutdown(); }

bool VkBloomPrefilterPipeline::Init(VkDevice device, VkRenderPass renderPass,
    ...
    if (vertModule == VK_NULL_HANDLE || fragModule == VK_NULL_HANDLE) {
        ...
        LOG_ERROR(Render, "VkBloomPrefilterPipeline: failed to create shader modules");
        return false;
    }
    ...
    if (vkCreateDescriptorSetLayout(device, &dslci, nullptr, &m_setLayout) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkBloomPrefilterPipeline: descriptor set layout failed");
        return false;
    }
    ...
    pushConstant.size = 8; // threshold (float), knee (float)
    ...
    if (vkCreatePipelineLayout(device, &plci, nullptr, &m_layout) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkBloomPrefilterPipeline: pipeline layout failed");
        return false;
    }
    ...
    if (vkCreateDescriptorPool(device, &dpci, nullptr, &m_descriptorPool) != VK_SUCCESS) {
        ...
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomPrefilterPipeline: descriptor pool failed");
        return false;
    }
    ...
    if (vkAllocateDescriptorSets(device, &dsai, &m_descriptorSet) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomPrefilterPipeline: allocate descriptor set failed");
        return false;
    }
    ...
    if (vkCreateSampler(device, &sci, nullptr, &m_sampler) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomPrefilterPipeline: sampler failed");
        return false;
    }
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &gpci, nullptr, &m_pipeline) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomPrefilterPipeline: graphics pipeline failed");
        return false;
    }
...
void VkBloomPrefilterPipeline::SetHDRView(VkImageView hdrView) {
...
void VkBloomPrefilterPipeline::Shutdown() {
