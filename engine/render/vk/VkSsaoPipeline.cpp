/**
 * @file VkSsaoPipeline.cpp
 * @brief Fullscreen SSAO: depth + normal -> occlusion 0..1 (view-space).
 *
...
/** Push constant: invProj (64) + proj (64) + view (64) = 192 bytes. */
constexpr VkDeviceSize kSsaoPushConstantSize = 192u;

VkSsaoPipeline::~VkSsaoPipeline() {
    Shutdown();
}

bool VkSsaoPipeline::Init(VkDevice device, VkRenderPass renderPass,
    ...
    if (vertModule == VK_NULL_HANDLE || fragModule == VK_NULL_HANDLE) {
        ...
        LOG_ERROR(Render, "VkSsaoPipeline: failed to create shader modules");
        return false;
    }
    ...
    if (vkCreateDescriptorSetLayout(device, &dslci, nullptr, &m_setLayout) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkSsaoPipeline: failed to create descriptor set layout");
        return false;
    }
    ...
    pushConstant.size       = static_cast<uint32_t>(kSsaoPushConstantSize);
    ...
    if (vkCreatePipelineLayout(device, &plci, nullptr, &m_layout) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkSsaoPipeline: failed to create pipeline layout");
        return false;
    }
    ...
    if (vkCreateDescriptorPool(device, &dpci, nullptr, &m_descriptorPool) != VK_SUCCESS) {
        ...
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkSsaoPipeline: failed to create descriptor pool");
        return false;
    }
    ...
    if (vkAllocateDescriptorSets(device, &dsai, &m_descriptorSet) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkSsaoPipeline: failed to allocate descriptor set");
        return false;
    }
    ...
    if (vkCreateSampler(device, &sci, nullptr, &m_sampler) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkSsaoPipeline: failed to create sampler");
        return false;
    }
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &gpci, nullptr, &m_pipeline) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkSsaoPipeline: failed to create graphics pipeline");
        return false;
    }
...
void VkSsaoPipeline::SetInputs(VkImageView depthView,
    ...
    normalImg.sampler     = m_sampler;
    normalImg.imageView   = normalView;
    normalImg.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

    VkDescriptorBufferInfo uboInfo{};
    uboInfo.buffer = kernelBuffer;
    uboInfo.offset = 0;
    uboInfo.range  = kernelBufferSize;

    VkDescriptorImageInfo noiseImg{};
    noiseImg.sampler     = noiseSampler;
    noiseImg.imageView   = noiseView;
    noiseImg.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
