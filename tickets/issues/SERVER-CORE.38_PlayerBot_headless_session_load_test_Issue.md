...
- src/shardd/playerbot/PlayerBotProfile.h

## Note
PlayerBot headless/strategies

---

## Contenu du ticket (SERVER-CORE.38)

# SERVER-CORE.38_PlayerBot_headless_session_load_test

## Objectif

Implémenter un mode **bot headless** côté shard LCDLLN, inspiré de
`src/game/PlayerBot` server-core. Adapté à LCDLLN : **pas un binaire séparé**,
...
- M00.1 (build base)
- SERVER-CORE.02 (Entities — Player headless)
- SERVER-CORE.04 (Movement — bot peut se déplacer)
- SERVER-CORE.07 (AI — bot a une stratégie)
...
- `SERVER-CORE_ANALYSIS.md` § PlayerBot (P3 shard)
- server-core `src/game/PlayerBot/Base/`, `AI/`
