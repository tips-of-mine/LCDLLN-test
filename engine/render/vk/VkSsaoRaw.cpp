/**
 * @file VkSsaoRaw.cpp
 * @brief Offscreen SSAO_Raw (R16_SFLOAT) for SSAO generate pass.
 *
...
void VkSsaoRaw::DestroyResources() {
...
bool VkSsaoRaw::Init(VkPhysicalDevice physicalDevice,
                     VkDevice         device,
                     uint32_t         width,
                     uint32_t         height) {
    ...
    if (vkCreateImage(device, &ici, nullptr, &m_image) != VK_SUCCESS) {
        LOG_ERROR(Render, "VkSsaoRaw: vkCreateImage failed");
        DestroyResources();
        return false;
    }
    ...
    if (allocInfo.memoryTypeIndex == UINT32_MAX ||
        ...
        LOG_ERROR(Render, "VkSsaoRaw: memory alloc/bind failed");
        DestroyResources();
        return false;
    }
    ...
    if (vkCreateImageView(device, &ivci, nullptr, &m_imageView) != VK_SUCCESS) {
        LOG_ERROR(Render, "VkSsaoRaw: vkCreateImageView failed");
        DestroyResources();
        return false;
    }
    ...
    if (vkCreateRenderPass(device, &rpci, nullptr, &m_renderPass) != VK_SUCCESS) {
        LOG_ERROR(Render, "VkSsaoRaw: vkCreateRenderPass failed");
        DestroyResources();
        return false;
    }
    ...
    if (vkCreateFramebuffer(device, &fbci, nullptr, &m_framebuffer) != VK_SUCCESS) {
        LOG_ERROR(Render, "VkSsaoRaw: vkCreateFramebuffer failed");
        DestroyResources();
        return false;
    }
    return true;
...
bool VkSsaoRaw::Recreate(uint32_t width, uint32_t height) {
    DestroyResources();
    m_extent = {width, height};
    if (width == 0 || height == 0) {
        return true;
