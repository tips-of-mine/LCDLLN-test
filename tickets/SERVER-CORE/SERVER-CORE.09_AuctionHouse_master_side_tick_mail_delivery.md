...
de `src/game/AuctionHouse` server-core. Master-side car cross-shard naturel
...
3. **Livraison via système mail** (SERVER-CORE.18) plutôt que retour direct
...
- M00.1 (build base)
- SERVER-CORE.13 (Database — SQLStorage et async)
- SERVER-CORE.18 (Mails) — livraison
...
7. Câbler livraison via Mail (dépendance SERVER-CORE.18).
...
- **Mail chain** : un winner reçoit 1 mail (item) ; un loser bidder reçoit 1 mail (refund de son bid) ; le seller reçoit 1 mail (gold). 3 mails par auction résolue. À 100k expirations/tick, ça fait 300k mails — batcher via `MassMailMgr` (SERVER-CORE.18).

## Références

- `SERVER-CORE_ANALYSIS.md` § AuctionHouse (P2 master)
- server-core `src/game/AuctionHouse/AuctionHouseMgr.cpp`,
