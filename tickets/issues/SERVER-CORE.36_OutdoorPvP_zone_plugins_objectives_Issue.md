...
- src/shardd/outdoorpvp/OutdoorPvPManager.h
- src/shared/network/OutdoorPvpPayloads.h

## Note
OutdoorPvP plugins/objectives

---

## Contenu du ticket (SERVER-CORE.36)

# SERVER-CORE.36_OutdoorPvP_zone_plugins_objectives

## Objectif

Mettre en place un framework **PvP en zones ouvertes** côté shard
LCDLLN, inspiré de `src/game/OutdoorPvP` server-core. Quatre piliers :
...
3. **WorldState broadcast** : compteurs de score diffusés via variables
   nommées (SERVER-CORE.28 World) à tous les clients de la zone.
...
- M00.1 (build base)
- SERVER-CORE.28 (World) — WorldState broadcast
- SERVER-CORE.07 (AI) — réactions des PNJ aux changements
- SERVER-CORE.16 (Globals/Conditions)
...
// Tous les clients dans la zone reçoivent automatiquement (SERVER-CORE.28).
```
...
- `SERVER-CORE_ANALYSIS.md` § OutdoorPvP (P3 shard)
- server-core `src/game/OutdoorPvP/OutdoorPvP.cpp`,
