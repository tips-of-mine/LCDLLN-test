...
côté shard LCDLLN, inspirée de `src/game/Globals` server-core. Quatre
piliers :
...
- M00.1 (build base)
- SERVER-CORE.13 (Database — SQLStorage pour cache)
- Pré-requis pour : SERVER-CORE.17 (Loot), SERVER-CORE.23 (Quests), SERVER-CORE.14 (DBScripts), SERVER-CORE.07 (AI/EventAI), SERVER-CORE.26 (Spells)
...
- `SERVER-CORE_ANALYSIS.md` § Globals (P2 shard)
- server-core `src/game/Globals/ObjectMgr.cpp`, `ObjectAccessor.cpp`,
