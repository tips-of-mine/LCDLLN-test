...
côté shard LCDLLN, inspiré de `src/game/World` server-core. Trois piliers :
...
- M00.1 (build base)
- SERVER-CORE.13 (Database — persistance)
- SERVER-CORE.16 (Globals/Conditions — alternative ciblée pour les prédicats simples)
- SERVER-CORE.19 (Maps — tick hiérarchique)
...
void World::Tick(int64 nowMs) {
  ...
  // 100ms : delegate to map tick (SERVER-CORE.19)
...
- **Compatibilité avec Conditions** : si `WorldStateExpression` est trop puissant et `Conditions` (SERVER-CORE.16) trop simple, on a deux DSL en parallèle. Choisir : `Conditions` pour les prédicats joueur (level, has_aura), `WorldStateExpression` pour les états globaux. Documenter clairement la séparation.
...
- `SERVER-CORE_ANALYSIS.md` § World (P2 shard)
- server-core `src/game/World/World.cpp`, `WorldState.cpp`,
