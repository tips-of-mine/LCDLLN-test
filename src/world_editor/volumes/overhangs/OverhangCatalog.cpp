...
namespace engine::editor::world::volumes::overhangs
{
	...
	bool OverhangCatalog::LoadFromContent(const std::string& contentRoot, std::string& outError)
	{
		m_entries.clear();
		const std::filesystem::path path =
			std::filesystem::path(contentRoot) / "meshes" / "overhangs" / "catalog.json";
		std::ifstream f(path);
		if (!f.good()) return true; // absent : pas une erreur
		std::stringstream buf;
		buf << f.rdbuf();
		return ParseJson(buf.str(), outError);
	}

	bool OverhangCatalog::ParseJson(const std::string& jsonText, std::string& outError)
	{
		m_entries.clear();
		size_t arrPos = 0u;
		if (!SeekKey(jsonText, 0u, jsonText.size(), "overhangs", arrPos))
		{
			return true; // pas de clé : catalogue vide
		}
		SkipWs(jsonText, arrPos);
		if (arrPos >= jsonText.size() || jsonText[arrPos] != '[')
		{
			outError = "OverhangCatalog: 'overhangs' is not an array";
			return false;
		}
		++arrPos;
		...
		while (arrPos < jsonText.size())
		{
			...
			if (arrPos >= jsonText.size() || jsonText[arrPos] != '{')
			{
				outError = "OverhangCatalog: expected '{'";
				return false;
			}
			...
			{
				outError = "OverhangCatalog: unmatched '{'";
				return false;
			}

			OverhangCatalogEntry entry;
			size_t kp = 0u;
			...
			if (!getStr("id", entry.id))      { outError = "OverhangCatalog: missing id"; return false; }
			...
			getVec3("wallAnchorPoint", entry.wallAnchorPoint);
			getVec3("wallNormalDirection", entry.wallNormalDirection);
			getF("coverageRadius", entry.coverageRadius);
	...
	const OverhangCatalogEntry* OverhangCatalog::FindById(const std::string& id) const
	{
		auto it = std::find_if(m_entries.begin(), m_entries.end(),
			[&id](const OverhangCatalogEntry& e) { return e.id == id; });
