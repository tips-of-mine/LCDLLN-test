...
LCDLLN, inspiré de `src/game/Pools` server-core. Trois piliers :
...
- M00.1 (build base)
- SERVER-CORE.02 (Entities) — pool spawne des Creature/GameObject
- SERVER-CORE.13 (Database — SQLStorage cache)
- SERVER-CORE.19 (Maps — pool persisté par instance via MapPersistentState)
...
via `MapPersistentState` (SERVER-CORE.19) — sauvegarde quels GUIDs sont
...
- **Persistance volatile vs persistante** : par défaut **volatile** côté `WorldMap`. La persistance arrive avec SERVER-CORE.19 (MapPersistentState) pour les raids.

## Références

- `SERVER-CORE_ANALYSIS.md` § Pools (P2 shard)
- server-core `src/game/Pools/PoolManager.cpp`
