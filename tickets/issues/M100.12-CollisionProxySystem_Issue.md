...
## Note
Proxies capsule/hull/trimesh + overlay

---

## Contenu du ticket (M100.12)
...
| Chemin | Rôle |
|--------|------|
| `src/client/world/collision/CollisionProxy.h/.cpp` | Structures + sérialisation. |
| `src/client/world/collision/AutoFitProxy.h/.cpp` | Heuristique d'auto-fit + V-HACD wrap. |
| `src/world_editor/panels/CollisionEditorPanel.h/.cpp` | UI. |
| `src/client/render/EditorOverlayPass.h/.cpp` | Passe overlay wireframe. |
| `src/client/render/shaders/editor_overlay.vert` | Shader minimal. |
| `src/client/render/shaders/editor_overlay.frag` | Shader minimal vert solide. |
| `src/client/world/collision/tests/CollisionProxyTests.cpp` | Tests. |

### Fichiers à modifier

| Chemin | Modification |
|--------|--------------|
| `src/client/render/FrameGraph.cpp` | `EditorOverlayPass` enregistrée **après** `Tonemap`, **uniquement** si `WorldEditorShell` actif (le frame graph la skippe sinon). |
| `src/world_editor/panels/AssetBrowserPanel.cpp` | À l'import, déclenche `AutoFitProxy`. |
| `src/world_editor/PlacementOverlapInfo.h/.cpp` | Compteur d'overlaps non bloquant. |
...
add_library(engine_core
    ...
    src/client/render/GeometryPass.cpp
    src/client/render/EditorOverlayPass.cpp
    ...
    src/world_editor/PlacementOverlapInfo.cpp
    ...
)
...
add_executable(collision_proxy_tests src/client/world/collision/tests/CollisionProxyTests.cpp)
