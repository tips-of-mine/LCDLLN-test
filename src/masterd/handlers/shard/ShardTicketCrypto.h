...
namespace engine::server
{
	...
	/// Computes HMAC over (ticket_id || account_id || target_shard_id || expires_at || character_id).
	/// \a character_id (TA.3) est inclus dans la couverture pour qu'il ne soit pas falsifiable.
	...
	/// Verifies HMAC of the given ticket fields (character_id inclus). Returns true if \a hmac matches.
