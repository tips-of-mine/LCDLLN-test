...
| Chemin | Rôle |
|--------|------|
| `src/client/audio/FootstepAudioSurfaceHook.h/.cpp` | Mappe `(SurfaceType, modifiers)` → audio event. |
| `src/world_editor/PlaytestMode.h/.cpp` | Gère entrée/sortie playtest. |
| `src/world_editor/PlaytestHUDOverlay.h/.cpp` | HUD debug. |
| `src/world_editor/tests/PlaytestTests.cpp` | Tests. |

### Fichiers à modifier

| Chemin | Modification |
|--------|--------------|
| `src/client/gameplay/CharacterController.cpp` | Hooks footstep events (déjà existants) → consomme `FootstepAudioSurfaceHook`. |
| `src/world_editor/WorldEditorShell.cpp` | Branche F5 sur `PlaytestMode::Toggle`. |
| `src/world_editor/EditorCameraController.cpp` | `Suspend()` / `Resume()` quand playtest. |
...
add_library(engine_core
    ...
    src/world_editor/PlaytestMode.cpp
    src/world_editor/PlaytestHUDOverlay.cpp
    ...
)
