/**
 * @file VkGeometryPipeline.cpp
 * @brief Geometry pass pipeline (3 color attachments + depth).
 */
...
/**
 * @brief Creates a VkShaderModule from SPIR-V bytes.
 */
...
VkGeometryPipeline::~VkGeometryPipeline() {
    Shutdown();
}
...
bool VkGeometryPipeline::Init(VkDevice device,
    ...
    if (vertModule == VK_NULL_HANDLE || fragModule == VK_NULL_HANDLE) {
        ...
        LOG_ERROR(Render, "VkGeometryPipeline: failed to create shader modules");
        return false;
    }
    ...
    if (vkCreatePipelineLayout(device, &plci, nullptr, &m_layout) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkGeometryPipeline: failed to create pipeline layout");
        return false;
    }
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &gpci, nullptr, &m_pipeline) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkGeometryPipeline: failed to create graphics pipeline");
        return false;
    }
...
void VkGeometryPipeline::Shutdown() {
