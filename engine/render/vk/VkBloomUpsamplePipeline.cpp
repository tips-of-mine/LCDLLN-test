/**
 * @file VkBloomUpsamplePipeline.cpp
 * @brief Bloom upsample: bilinear sample, additive blend (M08.2).
 */
...
VkBloomUpsamplePipeline::~VkBloomUpsamplePipeline() { Shutdown(); }

bool VkBloomUpsamplePipeline::Init(VkDevice device, VkRenderPass renderPass,
    ...
    if (vertModule == VK_NULL_HANDLE || fragModule == VK_NULL_HANDLE) {
        ...
        LOG_ERROR(Render, "VkBloomUpsamplePipeline: failed to create shader modules");
        return false;
    }
    ...
    if (vkCreateDescriptorSetLayout(device, &dslci, nullptr, &m_setLayout) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkBloomUpsamplePipeline: descriptor set layout failed");
        return false;
    }
    ...
    if (vkCreatePipelineLayout(device, &plci, nullptr, &m_layout) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkBloomUpsamplePipeline: pipeline layout failed");
        return false;
    }
    ...
    if (vkCreateDescriptorPool(device, &dpci, nullptr, &m_descriptorPool) != VK_SUCCESS) {
        ...
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomUpsamplePipeline: descriptor pool failed");
        return false;
    }
    ...
    if (vkAllocateDescriptorSets(device, &dsai, &m_descriptorSet) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomUpsamplePipeline: allocate descriptor set failed");
        return false;
    }
    ...
    if (vkCreateSampler(device, &sci, nullptr, &m_sampler) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomUpsamplePipeline: sampler failed");
        return false;
    }
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &gpci, nullptr, &m_pipeline) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkBloomUpsamplePipeline: graphics pipeline failed");
        return false;
    }
...
void VkBloomUpsamplePipeline::SetSourceView(VkImageView sourceView) {
...
void VkBloomUpsamplePipeline::Shutdown() {
