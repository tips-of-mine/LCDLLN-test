...
| Chemin | Rôle |
|--------|------|
| `src/client/world/wind/WindParams.h` | Struct shared C++ / GLSL. |
| `src/client/world/wind/WindZones.h/.cpp` | Sérialisation. |
| `src/client/world/wind/WindSystem.h/.cpp` | Évalue WindParams effectifs (global ± local). |
| `src/world_editor/WindZoneTool.h/.cpp` | Outil polygone. |
| `src/client/render/shaders/foliage.vert` | Shader vertex foliage avec vent. |
| `src/client/world/wind/tests/WindTests.cpp` | Tests. |

### Fichiers à modifier

| Chemin | Modification |
|--------|--------------|
| `src/client/render/GeometryPass.cpp` | Bind `WindParams` au pipeline foliage. |
| `src/client/app/Engine.cpp` | Update `timeSeconds` chaque frame. |
| `src/world_editor/panels/AtmospherePanel.h/.cpp` | Sous-panneau Wind. |
...
add_library(engine_core
    ...
    src/client/world/wind/WindZones.cpp
