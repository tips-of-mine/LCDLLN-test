...
namespace engine::world
{
	bool ParseMacroCellId(std::string_view s, int32_t& outRow, int32_t& outCol, std::string& err)
	{
		...
		outCol = 0;
		err.clear();
		// Minimum valide : R + ≥1 chiffre ligne + C + ≥1 chiffre colonne (ex. "R0C0" = 4 caractères).
		if (s.empty())
		{
			err = "macro cell id is empty";
			return false;
		}
		if (s[0] != 'R')
		{
		...
		if (s.size() < 4u)
		{
			err = "macro cell id too short (need at least R+digit+C+digit, e.g. R0C0)";
			return false;
		}
		size_t i = 1u;
