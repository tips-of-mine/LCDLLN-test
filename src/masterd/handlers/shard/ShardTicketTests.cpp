...
static std::vector<uint8_t> MakeValidTicket(std::string_view secret, uint64_t account_id, uint32_t target_shard_id, uint64_t expires_at, uint64_t character_id = 7777u)
{
	...
	if (!ComputeTicketHmac(ticket_id.data(), ticket_id.size(), account_id, target_shard_id, expires_at, character_id, secret, hmac.data(), hmac.size()))
		return {};
	return BuildShardTicketPayload(ticket_id, account_id, target_shard_id, expires_at, character_id, hmac.data(), hmac.size());
}
...
static void TestValidTicketAccepted()
{
	...
	std::vector<uint8_t> payload = MakeValidTicket(secret, 1001, 2, now + 60, 424242u);
	...
	Assert(accept->character_id == 424242u, "character_id correct (TA.3)");

	engine::core::Log::Shutdown();
}

// TA.3 : prouve que character_id est couvert par le HMAC — falsifier ses octets
// dans le ticket (offset 36 = 16 ticket_id + 8 account_id + 4 target_shard_id + 8 expires_at)
// doit invalider la signature et faire rejeter le ticket.
static void TestTamperedCharacterIdRefused()
{
	engine::core::LogSettings logSettings;
	logSettings.level = engine::core::LogLevel::Info;
	logSettings.console = false;
	...
	uint64_t now = static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::seconds>(
		std::chrono::system_clock::now().time_since_epoch()).count());
	std::vector<uint8_t> payload = MakeValidTicket(secret, 1005, 1, now + 60, 99u);
	Assert(!payload.empty(), "MakeValidTicket");
	const size_t kCharacterIdOffset = kShardTicketIdSize + 8u + 4u + 8u; // 36
	Assert(payload.size() > kCharacterIdOffset, "payload large enough to tamper character_id");
	payload[kCharacterIdOffset] ^= 0xFF; // change character_id sans recalculer le HMAC

	ShardTicketValidator validator;
	validator.SetSecret(secret);
	auto accept = validator.VerifyAndConsume(payload.data(), payload.size());
	Assert(!accept.has_value(), "Tampered character_id refused (HMAC covers it)");
...
int main()
{
	...
	TestTamperedCharacterIdRefused();
