...
Drapeaux clés (`src/client/AuthUi.h`) :
...
| Fichier | Rôle |
|---|---|
| `src/client/AuthUi.h` | Déclaration complète d'`AuthUiPresenter` : toutes les phases, membres, méthodes. **Lire en premier pour comprendre la structure.** |
...
### Struct centrale : `RenderModel` (dans `src/client/AuthUi.h`)
```
RenderModel
...
| Fichier | Rôle |
|---|---|
| `src/masterd/handlers/AuthRegisterHandler.h/.cpp` | Traite AUTH_REQUEST et REGISTER_REQUEST. Valide, hashe, crée compte. |
| `src/masterd/handlers/PasswordResetHandler.h/.cpp` | Reset mot de passe par email. |
| `src/masterd/handlers/CharacterCreateHandler.h/.cpp` | Création de personnage. |
| `src/masterd/handlers/ShardRegisterHandler.h/.cpp` | Enregistrement d'un shard auprès du master. |
| `src/masterd/handlers/ShardTicketHandler.h/.cpp` | Génération de ticket de connexion shard. |
| `src/masterd/handlers/ServerListHandler.h/.cpp` | Retourne la liste des shards disponibles. |
| `src/masterd/handlers/TermsHandler.h/.cpp` | Acceptation des CGU. |
...
| Fichier | Rôle |
|---|---|
| `src/client/LocalizationService.h/.cpp` | Charge les fichiers JSON de traduction, expose `Tr("clé")`. |
...
| `src/client/AuthUi.h` + `AuthUiPresenterCore.cpp` | Méthode publique `RequestReturnToLogin()` : reset `m_flowComplete=false`, repasse en `Phase::Login`. |
...
| `src/client/AuthUi.h` + `AuthScreenCharacterCreate.cpp` | Méthode `ImGuiSubmitCharacterCreate(cfg, name, raceId)` ; membre `m_characterRaceId` ; passé à `BuildCharacterCreateRequestPayload(name, raceId)`. |
...
| `src/masterd/handlers/CharacterCreateHandler.cpp` | Persiste `parsed->raceId` dans `characters.race_str` (colonne 0033). |
