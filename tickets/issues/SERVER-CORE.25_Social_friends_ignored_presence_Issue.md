...
- src/masterd/social/IgnoreList.h
- src/shared/network/IgnoreListPayloads.h

## Note
Social friends/ignore + payloads

---

## Contenu du ticket (SERVER-CORE.25)

# SERVER-CORE.25_Social_friends_ignored_presence

## Objectif

Mettre en place le **système social** (amis / ignorés / notes / présence)
côté master LCDLLN, inspiré de `src/game/Social` server-core. Quatre piliers :
...
- SERVER-CORE.01 (Chat) — pour intégration côté ChatGate (filtrage ignored)
...
l'envoi via le `ChatGate` (SERVER-CORE.01).
...
7. Câbler filtrage ignored dans `ChatGate` (SERVER-CORE.01).
8. Tests : 3 fichiers.
...
- **Friend mutual** : pas de confirmation requise (server-core style). Si LCDLLN veut Discord-style mutual, ajouter un état `Pending`.
...
- `SERVER-CORE_ANALYSIS.md` § Social (P2 master)
- server-core `src/game/Social/SocialMgr.cpp`, `SocialHandler.cpp`
