...
> Mise à jour 2026-07-10 — **Roadmap publique rattrapée (migration `0073_roadmap_items_v5.sql`)** : les grands systèmes de jeu livrés depuis mai (personnages, combat, sorts/compétences, quêtes + suivi HUD/XP, métiers récolte/artisanat, économie monnaie/marchands, commerce/hôtel des ventes, courrier, guildes/groupes/amis, fenêtre Personnage unifiée F1, météo/nuages volumétriques, anti-occlusion caméra, langue au 1er lancement, marqueurs de réapparition) sont désormais dans la table `roadmap_items`. **Nouveau système d'équipement d'objets (Chantier 2 SP-A)** : module partagé `src/shared/items/` (`ItemDefinition`/`ItemCatalog`, catalogue `game/data/items/items.json`), opcodes réseau `EquipRequest`=96 / `UnequipRequest`=97 / `EquipmentUpdate`=98 (bump `kProtocolVersion` 14→15), équipement serveur-autoritaire dans `ServerApp` (état `equipment[slot]`, bonus de stats dérivées), persistance KV + schéma `character_equipment`, panneau d'équipement + infobulles de bonus dans la fenêtre Personnage (client).
...
| `sql/migrations/000N_*.sql` | Migrations numérotées (0001 → 0073). Appliquées en ordre par lcdlln-master au démarrage. |
...
| `sql/migrations/0039_roadmap_racial_abilities.sql` | Roadmap publique v4 : capacités raciales design-validées (vol/téléport démon, monture dragon…) affichées « planifié ». |
| `sql/migrations/0072_factions_v2.sql` | Refonte des factions (ids courts, backfill) pour le système de personnages. |
| `sql/migrations/0073_roadmap_items_v5.sql` | Roadmap publique v5 : rattrapage des grands systèmes livrés (personnages, combat, compétences, quêtes, métiers, économie, social, fenêtre Personnage, météo, **équipement d'objets**). Repousse les items raciaux planifiés en fin de timeline. |
| `game/data/persistence/db/migrations/0002_characters_equipment.sql` | Schéma de l'équipement porté par personnage (`character_equipment`). Le store runtime shard est un fichier KV texte (clés `equipment.<slot>.item_id`) ; ce `.sql` versionne le schéma cible. |
| `src/masterd/migrations/MigrationRunner.h/.cpp` | Applique les migrations au démarrage du serveur (découverte auto par numéro de version). |
...
**Répétables / quotidiennes / auto-complete (EXT-2)** : une définition peut porter (tous **optionnels**) `repeat` ∈ {`none`(défaut), `repeatable`, `daily`, `weekly`, `cooldown`}, `cooldownHours` (entier, requis si `cooldown`), `autoComplete` (bool). Une quête `Completed` répétable **redevient disponible** quand sa borne est franchie : `Completed→Locked` puis repromue `Offered` par la ré-évaluation. Bornes : `daily` = minuit UTC, `weekly` = lundi 00:00 UTC, `cooldown` = N h après complétion, `repeatable` = immédiat (fonctions pures `UtcDayIndex`/`UtcWeekIndex`/`ShouldRepeatReset` + `QuestRuntime::ApplyRepeatResets`). Reset évalué **au login et au Talk** (V1 : pas de tick horaire → un joueur resté en ligne voit le reset au prochain Talk/relog). `autoComplete` : la quête se termine dès la dernière étape (pas de retour PNJ ; récompense versée via le helper partagé `ServerApp::GrantQuestReward`, réutilisé par le turn-in). Persistance : `QuestState.completedAtEpochMs` sérialisé (`quests.<i>.completed_at`), **`quests.format_version` bumpé 1→2** (compat : saves v1 → `completed_at=0`). **Aucun wire** (le client voit le statut via `QuestDelta`). Exemple : `kill_10_boars` = prime **quotidienne**. Détails : `docs/superpowers/specs/2026-07-03-quest-ext2-repeatable-design.md`.

**Partagées en groupe (EXT-3)** : champ **optionnel** `partyShared` (bool, défaut false). Quand un joueur déclenche un événement d'étape (kill/collect/talk/enter), les **membres de son groupe** (`PartySystem`, max 5 ; pas de raid distinct) situés dans la **même zone** et à moins de `server.quest.party_share_radius_m` mètres (config, défaut 30) sont **aussi crédités** — mais uniquement pour les quêtes `partyShared`. Mécanique : `QuestRuntime::ApplyEvent(..., onlyPartyShared=true)` (nouveau param défauté qui filtre les défs non-partagées) ; `ServerApp::ApplyQuestEvent` fait le fan-out vers chaque coéquipier via `FindConnectedClient` + garde zone-avant-distance (`DistanceSquaredXZ`) ; post-traitement factorisé dans `ServerApp::FinalizeQuestDeltas` (réutilisé acteur + coéquipiers). **Chacun garde sa propre progression et sa propre récompense** (pas de partage de butin). **Tous les objectifs** partagés. **Aucun wire** (chaque coéquipier reçoit son `QuestDelta`), pas de persistance nouvelle. Détails : `docs/superpowers/specs/2026-07-03-quest-ext3-party-shared-design.md`.
