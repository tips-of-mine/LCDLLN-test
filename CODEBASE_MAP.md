...
## 22. Entities foundation (Wave 7, #575)

### Base actuelle (livrée)

| Fichier | Rôle |
|---------|------|
| `src/shardd/entities/Object.{h,cpp}` | Base abstraite : `ObjectGuid m_guid`, virtual `Update(diff)`, helpers UpdateMask. |
| `src/shardd/entities/ObjectGuid.h` | GUID 64 bits : low 32 = entry, high 32 = type+counter. Constexpr helpers. |
| `src/shardd/entities/UpdateField.h` | Enum centralisé des indices UpdateFields (ObjectFields, UnitFields, PlayerFields, GameObjectFields). |
| `src/shardd/entities/UpdateMask.h` | Bitmask delta : marque les fields modifiés, génère un buffer compact à la sérialisation. |

### Roadmap (Wave 17)

Manquent (à ajouter sur la base Wave 7) :
- `Unit.{h,cpp}` (HP, stats, faction, posture combat)
- `Player.{h,cpp}` (extends Unit, inventaire, quêtes, social)
- `WorldObject.{h,cpp}` (GameObject statique, transport futur)
- `Creature.{h,cpp}` (extends Unit, AI hookée)

Voir [`docs/superpowers/specs/2026-05-11-waves-17-38-server-foundations-design.md`](docs/superpowers/specs/2026-05-11-waves-17-38-server-foundations-design.md) §4 Wave 17.
