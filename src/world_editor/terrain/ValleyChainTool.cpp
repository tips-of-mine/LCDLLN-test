...
namespace engine::editor::world
{
	...
	bool ValleyChainTool::Init(CommandStack& stack, TerrainDocument& doc,
	...
	void ValleyChainTool::AddVertex(float worldX, float worldZ)
	{
		if (m_params.vertices.size() >= kMacroPolylineMaxVertices) return;
		PolylineVertex v;
		v.worldX = worldX;
		v.worldZ = worldZ;
		// Defaults adaptés à une vallée : moins large, moins profonde,
		// asymétrie nulle. L'utilisateur ajuste ensuite via le panel.
		if (m_params.vertices.empty())
		{
			v.widthMeters    = 200.0f;
			v.heightMeters   = 150.0f;
			v.noiseAmplitude =  15.0f;
		}
		else
		{
	...
	void ValleyChainTool::RemoveVertex(size_t idx)
	{
	...
	void ValleyChainTool::MoveVertex(size_t idx, float worldX, float worldZ)
	{
		if (idx >= m_params.vertices.size()) return;
	...
	void ValleyChainTool::ToggleLoop()
	{
	...
	void ValleyChainTool::SetGlobalParams(FlankProfile profile,
	...
	SparseChunkDeltas ValleyChainTool::BuildDeltas() const
	{
		return RasterizeMacroPolyline(m_params, /*invert=*/true);
	}

	bool ValleyChainTool::Apply()
	{
		...
		auto cmd = std::make_unique<ValleyChainCommand>(*m_doc, std::move(deltas));
		m_stack->Push(std::move(cmd));
		Reset();
		return true;
	}

	void ValleyChainTool::Cancel()
	{
		Reset();
	}
}
