...
namespace engine::client
{
	namespace
	{
		...
		/// Parseur JSON minimal local au catalogue — même convention que les
		/// autres loaders du repo (SpawnerRuntime, SkillSystem) : parseur par
		/// module, aucune dépendance nouvelle.
	...
	bool CreatureCatalog::Init(const engine::core::Config& config)
	{
		...
		if (jsonText.empty())
		{
			LOG_WARN(Core, "[CreatureCatalog] Init skipped: empty or missing file ({}) — mobs rendus en fallback", relativePath);
			return false;
		}
		...
		{
			LOG_WARN(Core, "[CreatureCatalog] Init FAILED: {} ({}) — mobs rendus en fallback", loadError, relativePath);
			return false;
		}

		LOG_INFO(Core, "[CreatureCatalog] Init OK (archetypes={})", m_appearances.size());
		return true;
	}

	const CreatureAppearance* CreatureCatalog::Find(uint32_t archetypeId) const
	{
		const auto it = m_appearances.find(archetypeId);
	...
	bool CreatureCatalog::LoadFromText(std::string_view jsonText, std::string& outError)
	{
		m_appearances.clear();
		...
		for (size_t index = 0; index < archetypesValue->arrayValue.size(); ++index)
		{
			...
			if (idValue == nullptr || !TryGetUint(*idValue, archetypeId) || archetypeId == 0)
			{
			...
			const std::string entryLabel = "archetype id=" + std::to_string(archetypeId);
			...
			appearance.name = nameValue->stringValue;

			const JsonValue* levelValue = FindObjectMember(entry, "level");
			if (levelValue == nullptr || !TryGetUint(*levelValue, appearance.level) || appearance.level == 0)
			{
			...
			appearance.meshKey = meshValue->stringValue;

			const JsonValue* scaleValue = FindObjectMember(*modelValue, "scale");
			if (scaleValue != nullptr)
			{
				if (!TryGetFloat(*scaleValue, appearance.scale) || appearance.scale <= 0.0f)
				{
			...
			if (!m_appearances.emplace(archetypeId, std::move(appearance)).second)
			{
