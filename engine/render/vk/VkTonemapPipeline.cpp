/**
 * @file VkTonemapPipeline.cpp
 * @brief Fullscreen tonemap: sample HDR, output LDR.
 */
...
VkTonemapPipeline::~VkTonemapPipeline() {
    Shutdown();
}

bool VkTonemapPipeline::Init(VkDevice device, VkRenderPass renderPass,
    ...
    if (vertModule == VK_NULL_HANDLE || fragModule == VK_NULL_HANDLE) {
        ...
        LOG_ERROR(Render, "VkTonemapPipeline: failed to create shader modules");
        return false;
    }
    ...
    if (vkCreateDescriptorSetLayout(device, &dslci, nullptr, &m_setLayout) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkTonemapPipeline: failed to create descriptor set layout");
        return false;
    }
    ...
    if (vkCreatePipelineLayout(device, &plci, nullptr, &m_layout) != VK_SUCCESS) {
        ...
        LOG_ERROR(Render, "VkTonemapPipeline: failed to create pipeline layout");
        return false;
    }
    ...
    if (vkCreateDescriptorPool(device, &dpci, nullptr, &m_descriptorPool) != VK_SUCCESS) {
        ...
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkTonemapPipeline: failed to create descriptor pool");
        return false;
    }
    ...
    if (vkAllocateDescriptorSets(device, &dsai, &m_descriptorSet) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkTonemapPipeline: failed to allocate descriptor set");
        return false;
    }
    ...
    if (vkCreateSampler(device, &sci, nullptr, &m_sampler) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkTonemapPipeline: failed to create sampler");
        return false;
    }
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &gpci, nullptr, &m_pipeline) != VK_SUCCESS) {
        ...
        m_layout = VK_NULL_HANDLE;
        m_setLayout = VK_NULL_HANDLE;
        LOG_ERROR(Render, "VkTonemapPipeline: failed to create graphics pipeline");
        return false;
    }
...
void VkTonemapPipeline::SetHDRView(VkImageView hdrView) {
...
void VkTonemapPipeline::Shutdown() {
