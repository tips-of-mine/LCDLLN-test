...
## Note
Lacs/rivieres + mesh + flow

---

## Contenu du ticket (M100.13)
...
| Chemin | Rôle |
|--------|------|
| `src/client/world/water/WaterSurfaces.h/.cpp` | Structures `LakeInstance`, `RiverInstance`, sérialisation. |
| `src/client/world/water/WaterMeshBuilder.h/.cpp` | Triangulation Delaunay 2D (lac) + ribbon (rivière). |
| `src/world_editor/LakeTool.h/.cpp` | Outil édition lac. |
| `src/world_editor/RiverTool.h/.cpp` | Outil édition rivière. |
| `src/world_editor/WaterDocument.h/.cpp` | État monde. |
| `src/client/world/water/tests/WaterSurfacesTests.cpp` | Tests + parité. |

### Fichiers à modifier

| Chemin | Modification |
|--------|--------------|
| `src/client/world/StreamCache.cpp` | `LoadWater(zone)`. |
