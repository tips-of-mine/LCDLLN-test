...
- src/client/world/terrain/TerrainChunk.h
- src/client/world/terrain/tests/TerrainChunkTests.cpp

## Note
Heightmap 257x257 + mesh builder

---

## Contenu du ticket (M100.5)
...
| Chemin | Rôle |
|--------|------|
| `src/client/world/terrain/TerrainChunk.h` | Structure + constantes + helpers (sample, gradient). |
| `src/client/world/terrain/TerrainChunk.cpp` | Implémentation (lecture/écriture binaire, validation). |
| `src/client/world/terrain/TerrainChunkLoader.h/.cpp` | Adapter `StreamCache`. |
| `src/client/world/terrain/TerrainMeshBuilder.h/.cpp` | Conversion heightmap → vertex/index buffers. |
| `src/world_editor/TerrainDocument.h/.cpp` | Wrapper éditeur. |
| `src/client/world/terrain/tests/TerrainChunkTests.cpp` | Tests unitaires. |
| `src/client/world/terrain/tests/TerrainParityTests.cpp` | Round-trip éditeur ↔ client. |

### Fichiers à modifier

| Chemin | Modification |
|--------|--------------|
| `src/client/world/StreamCache.h/.cpp` | Ajouter `LoadTerrainChunk(GlobalChunkCoord)` qui retourne un `std::shared_ptr<TerrainChunk>`. |
| `src/client/world/StreamingScheduler.cpp` | Inclure `terrain.bin` dans le set de fichiers requis par chunk. |
| `src/client/render/GeometryPass.cpp` | Lire les `TerrainMeshGpu` du chunk si présent et soumettre le drawcall. |
...
| Étape | Code | Chemin |
|-------|------|--------|
| Éditeur écrit | `engine::editor::world::TerrainDocument::SaveToDisk` | `src/world_editor/TerrainDocument.cpp` |
| Sérialisation | `engine::world::terrain::SaveTerrainBin` | `src/client/world/terrain/TerrainChunk.cpp` |
| Client lit | `engine::world::terrain::TerrainChunkLoader::Load` via `StreamCache::LoadTerrainChunk` | `src/client/world/terrain/TerrainChunkLoader.cpp`, `src/client/world/StreamCache.cpp` |
...
add_executable(terrain_parity_tests
    src/client/world/terrain/tests/TerrainParityTests.cpp)
...
- [ ] `src/masterd/` ne contient aucune référence à `TerrainChunk`.
