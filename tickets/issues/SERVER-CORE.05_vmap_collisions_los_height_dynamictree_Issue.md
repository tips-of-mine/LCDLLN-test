...
shard LCDLLN, inspiré de `src/game/vmap` server-core, pour répondre aux
besoins de gameplay :
...
l'analyse server-core. Sans ça, pas de combat distance crédible, pas de
portes vraiment fermées, anticheat de mouvement très limité.

## Dépendances

- M00.1 (build base)
- SERVER-CORE.02 (Entities) — `WorldObject::CanSeeObject(target)` câble vmap
- SERVER-CORE.03 (Grids) — déclenche le streaming des tiles vmap selon les
  cellules `Active`
...
- `VMapStreamer.{h,cpp}` — réagit aux transitions `Loaded ↔ Active`
  des cellules grid (SERVER-CORE.03) pour `acquire()` / `release()` les
  tiles.
...
Ces 3 requêtes couvrent 95% des besoins gameplay (cf. server-core).
...
Câblé sur les transitions de `GridState` (SERVER-CORE.03) :
...
- **Pas de glm en dépendance shard** : le client utilise probablement glm pour Vector3, mais le shard n'a pas besoin de la lib complète. Réutiliser `MovementTypedefs.h` (SERVER-CORE.04) qui définit `Vector3` localement, ou créer un `engine/math/Geom.h` minimal côté serveur.

## Références

- `SERVER-CORE_ANALYSIS.md` § vmap (P1 shard)
- server-core `src/game/vmap/VMapManager2.cpp`, `BIH.h`,
...
- À coordonner avec : `engine/render/terrain/` côté client (cohérence du
  terrain), SERVER-CORE.13 (Maps), SERVER-CORE.03 (Grids streaming)
