...
- src/masterd/chat/ChatGate.h
- src/masterd/chat/ChatSanitizer.h

## Note
Chat routing/safety/commands + tests

---

## Contenu du ticket (SERVER-CORE.01)

# SERVER-CORE.01_Chat_routing_safety_commands_split
...
de server-core `src/game/Chat`. Trois objectifs simultanés :
...
Note : `AccountRole` vient du ticket Accounts (SERVER-CORE.06) — câbler une fois ce dernier livré.
...
- `SERVER-CORE_ANALYSIS.md` § Chat (P1 cross master+shard)
- server-core `src/game/Chat/Chat.cpp`, `src/game/Chat/ChannelMgr.cpp`, `src/game/Chat/Level0.cpp` à `Level3.cpp`
