...
namespace engine::editor::world::volumes::dungeons
{
	namespace
	{
		...
		bool ReadJsonNumber(const std::string& s, size_t& p, double& out)
		{
			SkipWs(s, p);
	...
	bool DungeonCatalog::LoadFromContent(const std::string& contentRoot, std::string& outError)
	{
		m_entries.clear();
		const std::filesystem::path path =
			std::filesystem::path(contentRoot) / "meshes" / "dungeons" / "catalog.json";
		std::ifstream f(path);
	...
	bool DungeonCatalog::ParseJson(const std::string& jsonText, std::string& outError)
	{
		m_entries.clear();
		size_t arrPos = 0u;
		if (!SeekKey(jsonText, 0u, jsonText.size(), "dungeons", arrPos)) return true;
		SkipWs(jsonText, arrPos);
		if (arrPos >= jsonText.size() || jsonText[arrPos] != '[')
		{
			outError = "DungeonCatalog: 'dungeons' is not an array";
			return false;
		}
		++arrPos;
		while (arrPos < jsonText.size())
		{
			...
			if (arrPos >= jsonText.size() || jsonText[arrPos] != '{')
			{
				outError = "DungeonCatalog: expected '{'";
				return false;
			}
			...
			{
				outError = "DungeonCatalog: unmatched '{'";
				return false;
			}

			DungeonCatalogEntry entry;
			size_t kp = 0u;
			...
			auto getU16 = [&](const std::string& key, uint16_t& dst, bool optional = true) {
				if (SeekKey(jsonText, objStart, objEnd, key, kp))
				{
					double v = 0.0;
					if (!ReadJsonNumber(jsonText, kp, v)) return false;
					dst = static_cast<uint16_t>(v);
					return true;
				}
				return optional;
			};
			auto getU8 = [&](const std::string& key, uint8_t& dst, bool optional = true) {
				if (SeekKey(jsonText, objStart, objEnd, key, kp))
				{
					double v = 0.0;
					if (!ReadJsonNumber(jsonText, kp, v)) return false;
			...
			if (!getStr("id", entry.id))  { outError = "DungeonCatalog: missing id"; return false; }
			if (!getStr("displayName", entry.displayName, true)) entry.displayName = entry.id;
			if (!getStr("description", entry.description, true)) entry.description.clear();
			if (!getStr("decorativeMesh", entry.decorativeMeshPath, true)) entry.decorativeMeshPath.clear();
			if (!getStr("thumbnail", entry.thumbnailPath, true)) entry.thumbnailPath.clear();
			getU16("requiredLevel", entry.requiredLevel);
			getU8("minDifficulty", entry.minDifficulty);
			getU8("maxDifficulty", entry.maxDifficulty);
			if (entry.maxDifficulty < entry.minDifficulty)
				entry.maxDifficulty = entry.minDifficulty;
	...
	const DungeonCatalogEntry* DungeonCatalog::FindById(const std::string& id) const
	{
		auto it = std::find_if(m_entries.begin(), m_entries.end(),
			[&id](const DungeonCatalogEntry& e) { return e.id == id; });
