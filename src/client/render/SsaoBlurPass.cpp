...
namespace engine::render
{
	...
	bool SsaoBlurPass::Init(VkDevice device, VkPhysicalDevice /*physicalDevice*/,
		...
		VkPipelineCache pipelineCache)
	{
		...
		{
			LOG_ERROR(Render, "SsaoBlurPass::Init: invalid arguments");
			return false;
		}
		...
		{
			LOG_ERROR(Render, "SsaoBlurPass: vkCreateRenderPass failed");
			return false;
		}
		...
		{
			LOG_ERROR(Render, "SsaoBlurPass: vkCreateDescriptorSetLayout failed");
			Destroy(device);
			return false;
		}
		...
		{
			LOG_ERROR(Render, "SsaoBlurPass: vkCreateDescriptorPool failed");
			Destroy(device);
			return false;
		}
		...
		{
			LOG_ERROR(Render, "SsaoBlurPass: vkAllocateDescriptorSets failed");
			Destroy(device);
			return false;
		}
		...
		{
			LOG_ERROR(Render, "SsaoBlurPass: vkCreateSampler failed");
			Destroy(device);
			return false;
		}
		...
		pushRange.offset = 0;
		pushRange.size = sizeof(BlurParams);
		...
		{
			LOG_ERROR(Render, "SsaoBlurPass: vkCreatePipelineLayout failed");
			Destroy(device);
			return false;
		}
		...
		{
			LOG_ERROR(Render, "SsaoBlurPass: vkCreateGraphicsPipelines failed");
		...
		LOG_INFO(Render, "SsaoBlurPass: initialised");
		return true;
	}

	void SsaoBlurPass::Record(VkDevice device, VkCommandBuffer cmd, Registry& registry, VkExtent2D extent,
		...
		bool horizontal, uint32_t frameIndex)
	{
		...
		VkImageView viewAo = registry.getImageView(idInputAo);
		VkImageView viewDepth = registry.getImageView(idDepth);
		VkImageView viewOut = registry.getImageView(idOutput);
		if (viewAo == VK_NULL_HANDLE || viewDepth == VK_NULL_HANDLE || viewOut == VK_NULL_HANDLE)
			return;

		uint32_t setIdx = frameIndex % m_maxFrames;
		VkDescriptorSet ds = m_descriptorSets[setIdx];

		VkDescriptorImageInfo aoImg = { m_sampler, viewAo, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
		...
		BlurParams params{};
		params.texelSizeX = 1.0f / static_cast<float>(extent.width);
		params.texelSizeY = 1.0f / static_cast<float>(extent.height);
		...
		vkCmdPushConstants(cmd, m_pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(BlurParams), &params);
