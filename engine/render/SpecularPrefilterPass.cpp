...
namespace engine::render
{
		...
		uint32_t queueFamilyIndex)
	{
		...
		// ---------------------------------------------------------------------
		// Cubemap image (6 layers, mipCount levels)
		// ---------------------------------------------------------------------
		...
		{
			LOG_ERROR(Render, "SpecularPrefilterPass: no suitable memory type");
		...
		if (vkAllocateMemory(device, &memAlloc, nullptr, &m_memory) != VK_SUCCESS)
		{
