...
- src/masterd/reputation/ReputationManager.h
- src/masterd/reputation/ReputationManagerTests.cpp

## Note
Reputation matrix + spillover

---

## Contenu du ticket (SERVER-CORE.24)

# SERVER-CORE.24_Reputation_faction_template_matrix

## Objectif

Mettre en place le **système de réputation joueur ↔ factions** côté
shard LCDLLN, inspiré de `src/game/Reputation` server-core. Cinq piliers :
...
- SERVER-CORE.02 (Entities — Unit a `factionId`)
- SERVER-CORE.11 (Combat — `CanAttack(other)` consulte les flags)
...
- `SERVER-CORE_ANALYSIS.md` § Reputation (P2 shard)
- server-core `src/game/Reputation/ReputationMgr.cpp`,
