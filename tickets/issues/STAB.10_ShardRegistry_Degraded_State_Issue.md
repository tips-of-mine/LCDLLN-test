...
## Rapport final (STAB.10)

### 1) FICHIERS

- **Créés :**
  - `tickets/issues/STAB.10_ShardRegistry_Degraded_State_Issue.md`

- **Modifiés :**
  - `engine/server/ShardRegistry.h` (SetDegradedLoadThreshold, SetShardDegradedCallback, m_degraded_load_threshold, m_shard_degraded_callback)
  - `engine/server/ShardRegistry.cpp` (implémentation des deux setters ; dans UpdateHeartbeat : transitions Online↔Degraded selon ratio, logs et callback hors lock)
  - `engine/server/main_server_linux.cpp` (config `shard.degraded_load_threshold`, SetDegradedLoadThreshold, SetShardDegradedCallback)
  - `engine/server/ShardDownDetectionTests.cpp` (4 tests : Online→Degraded, Degraded→Online, Degraded→Offline par timeout, SelectShard exclut Degraded)
...
Tests unitaires (shard down + degraded) :

```bat
.\build\vs2022-x64\Release\shard_down_detection_tests.exe
```

Ou via ctest :

```bat
ctest --test-dir build/vs2022-x64 -C Release -R shard_down_detection_tests
```
...
- Structure du repo respectée (uniquement `engine/server` et `tickets/issues` modifiés).
- Portée limitée au ticket (transitions Degraded, config, callback, tests).
- Logs requis présents (now degraded, recovered to online) ; callback invoquée hors lock.
- Aucune modification de SelectShard au-delà de l’exclusion existante ; EvictStaleHeartbeats inchangé (Degraded déjà traité comme non-Offline).
